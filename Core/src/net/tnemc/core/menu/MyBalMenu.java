...
public class MyBalMenu extends Menu {
  ...
  static Optional<Receipt> processTransaction(final PlayerProvider player, final Transaction transaction, final String modifiedAccount, final BigDecimal modifier) {
  ...
  protected void actionsPage(final PageOpenCallback callback) {
    ...
    MyBalMenuView.actionsPage(callback, this.name);
  }
  ...
  protected Icon buildBalanceIcon(final int slot, final Currency currency, final Account account) {
    ...
    return MyBalMenuView.buildBalanceIcon(this.name, slot, currency, account);
  }

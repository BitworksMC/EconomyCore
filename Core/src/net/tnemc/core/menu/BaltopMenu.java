...
public class BaltopMenu extends Menu {
  ...
  public void handleMainPage(final PageOpenCallback callback) {
    ...
    final int page = callback.getPlayer().viewer().isPresent()
                     ? (Integer)callback.getPlayer().viewer().get().dataOrDefault(TOP_PAGE_ID, 1) : 1;
    addNavigation(callback, id, page, maxPages);
    addTransactions(callback, account.get(), sorted, page);
  }
  ...
  private void addTransactions(final PageOpenCallback callback, final Account account,
                               final SortedHistory sorted, final int page) {
